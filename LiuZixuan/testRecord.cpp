...
public:
	...
	void Select(Table &t, std::vector<SQLCondition> &cond, std::vector<Tuple> &tuple);
...
bool MatchSQLCondition(Table &t, Tuple &tuple, std::vector<SQLCondition> &cond)
{
...
void RecordManager::Select(Table &t, std::vector<SQLCondition> &cond, std::vector<Tuple> &tuples)
{
	...
	for (int i = 0; i < t.blockNum; i++) {
		int start = 0;
		BufferID = bum.Load(t.tablename+".data", i);
		...
		while (str[0] != END && start <= BLOCKSIZE-t.tupleLength) {
			...
			if (str[0] != EMPTY) {
				...
				cout << "After Split: " <<temp_tuple.columns[1] << "\n";
...
{
	BufferPosition bp = bum.GetInsertPosition(t);
	string str;
	stringstream S(str);
	int i = 0;
	int tmp_int;
	...
	for (auto it = t.attributes.begin(); it != t.attributes.end(); ++it) {
		switch (it->type) {
			case INT:
				tmp_int = atoi(tuple.columns[i].c_str());
				S.write((char *)&tmp_int, sizeof(int));
				break;
			case CHAR:
				S.write(tuple.columns[i].c_str(), it->length);
//				cout << "it->length" << it->length << "\n";
				break;
			case FLOAT:
				tmp_float = atof(tuple.columns[i].c_str());
				S.write((char *)&tmp_float, sizeof(float));
				break;
			default:
	...
	S.read(p, t.tupleLength);
	p[t.tupleLength] = END;
	bum.bufferblocks[bp.BufferID].modified = true;
	...
	for (int i = 0; i < t.blockNum; i++) {
		...
		while (str[0] != END && start <= BLOCKSIZE - t.tupleLength) {
			...
			if (str[0] != EMPTY) {
				...
				if (MatchSQLCondition(t, temp_tuple, cond)) {
					tuples.push_back(temp_tuple);
					DeleteData(bum.bufferblocks[BufferID].contents(), start, t.tupleLength);
					bum.bufferblocks[BufferID].modified = true;
				}	
			}
			start += t.tupleLength;
			str = ReadData(bum.bufferblocks[BufferID].contents(), start, t.tupleLength);
...
int main()
{
    ...
    t.blockNum = 1;
    t.attriNum = 3;
    t.tupleLength = sizeof(int)+50+sizeof(float);
    t.tupleNum = 10;
    t.attributes.push_back(ID);
    t.attributes.push_back(Name);
    t.attributes.push_back(Fen);

//    string the_name = "ylirlwnqsa";

    vector<SQLCondition> cond;
    SQLCondition temp_cond;
    temp_cond.column = 2;
    temp_cond.ope = Nogreater;
    temp_cond.value = "6.0";
    cond.push_back(temp_cond);
    
    vector<SQLCondition> a;
    temp_cond.column = 1;
    temp_cond.ope = Equal;
    temp_cond.value = "Liuzx";
    a.push_back(temp_cond);

    vector<Tuple> tuples;
    
    Tuple temp_tuple;
    temp_tuple.columns.push_back("12345");
    temp_tuple.columns.push_back("Liuzx");
    temp_tuple.columns.push_back("5.0");
    
    int bid = bum.Load("student.data", 0);
	printblock(bum.GetBuffer()[0].contents());
    rem.Insert(temp_tuple, t);
    printblock(bum.GetBuffer()[0].contents());
    
//	rem.Delete(t, a, tuples);
    rem.Select(t, cond, tuples);
    
    cout << tuples.size() << endl;
    for (int j = 0; j < tuples.size(); j++)
    for (int i = 0; i < tuples[j].columns.size(); i++) {
    	cout << tuples[j].columns[i] << "\n";
	}
